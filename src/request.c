...
void acceptGroupReport( uint32_t src, uint32_t group )
{
    ...
    if ( sourceVif->state == IF_STATE_DOWNSTREAM ) {
        ...
        my_log( LOG_DEBUG, 0, "Should insert group %s (from: %s) to route table. Vif Ix : %d", inetFmt( group, s1 ),
                inetFmt( src, s2 ), sourceVif->vifindex );
        ...
        my_log( LOG_INFO, 0, "The group address %s may not be requested from this interface. Ignoring.",
                inetFmt( group, s1 ) );
    }
    else {
...
void acceptLeaveMessage( uint32_t src, uint32_t group )
{
    ...
    my_log( LOG_DEBUG, 0, "Got leave message from %s to %s. Starting last member detection.", inetFmt( src, s1 ),
            inetFmt( group, s2 ) );
...
void sendGroupSpecificMemberQuery( void *argument )
{
     ...
     *        It might be better to send only a query on the interface the leave was accepted on and remove only that
     * interface from the route.
     */
        ...
        if ( Dp->InAdr.s_addr && !( Dp->Flags & IFF_LOOPBACK ) ) {
            if ( Dp->state == IF_STATE_DOWNSTREAM ) {
                ...
                my_log( LOG_DEBUG, 0, "Sent membership query from %s to %s. Delay: %d", inetFmt( Dp->InAdr.s_addr, s1 ),
                        inetFmt( allhosts_group, s2 ), conf->queryResponseInterval );
            }
        }
    }
