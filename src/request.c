...
void acceptGroupReport(uint32_t src, uint32_t group) {
    ...
    // we have a IF so check if that it's an downstream IF.
    if (sourceVif->state == IF_STATE_DOWNSTREAM) {
        ...
        // if we don't have a whitelist we insertRoute and are done
        ...
        my_log(LOG_INFO, 0,
               "The group address %s may not be requested from this interface. Ignoring.",
               inetFmt(group, s1));
    } else {
...
void acceptLeaveMessage(uint32_t src, uint32_t group) {
    ...
    my_log(LOG_DEBUG, 0, "Got leave message from %s to %s. Starting last member detection.",
            inetFmt(src, s1),
            inetFmt(group, s2));
    ...
    // We have an IF so check that it's a downstream IF.
    if (sourceVif->state == IF_STATE_DOWNSTREAM) {
...
void sendGroupSpecificMemberQuery(void *argument) {
    ...
    /**
     * FIXME:
     * This loops through all interfaces the group is active on and sends
     * queries.
            ...
            if (Dp->state == IF_STATE_DOWNSTREAM) {
                ...
                if (interfaceInRoute(gvDesc->group, Dp->index)) {
                    ...
                    sendIgmp(Dp->InAdr.s_addr, gvDesc->group, IGMP_MEMBERSHIP_QUERY,
                             conf->lastMemberQueryInterval * IGMP_TIMER_SCALE, gvDesc->group, 0);

                    my_log(LOG_DEBUG, 0, "Sent membership query from %s to %s. Delay: %d",
                           inetFmt(Dp->InAdr.s_addr, s1), inetFmt(gvDesc->group, s2), conf->lastMemberQueryInterval);
                }
            }
        }
    }
...
void sendGeneralMembershipQuery(void) {
    ...
    for (Ix = 0; (Dp = getIfByIx(Ix)); Ix++) {
        if (Dp->InAdr.s_addr && !(Dp->Flags & IFF_LOOPBACK)) {
            if (Dp->state == IF_STATE_DOWNSTREAM) {
                // send membership query on each downstream interface
                sendIgmp(Dp->InAdr.s_addr, allhosts_group, IGMP_MEMBERSHIP_QUERY,
