...
**  igmp.c
**
**  Receive IGMP requests, and handle them appropriately...
**----------------------------------------------------------------------------*/
...
uint32_t alligmp3_group;    // All IGMPv3 addr in net order
...
/**
** Processes a newly received IGMP packet that is sitting in the input
** packet buffer.
*/
void acceptIgmp(int recvlen) {
    ...
    ** this multicast address is used by UPnP/SSDP.
    */
                ...
                if (-1 != upStreamIfIdx[i]) {
                    ...
                    if (checkVIF == 0) {
                        ...
                        if (downVIF && downVIF->state & IF_STATE_DOWNSTREAM) {
                            my_log(LOG_NOTICE, 0,
                                    "The source address %s for group %s is from downstream VIF[%d]. Ignoring.",
                                    inetFmt(src, s1),
                                    inetFmt(dst, s2), i);
                        } else {
                            my_log(LOG_WARNING, 0,
                                    "The source address %s for group %s, is not in any valid net for upstream VIF[%d].",
                                    inetFmt(src, s1),
                                    inetFmt(dst, s2),
                                    i);
                        }
    ...
    if ((ipdatalen < IGMP_MINLEN) || (igmp->igmp_type == IGMP_V3_MEMBERSHIP_REPORT && ipdatalen <= IGMPV3_MINLEN)) {
        my_log(LOG_WARNING, 0, "received IP data field too short (%u bytes) for IGMP, from %s",
                ipdatalen,
                inetFmt(src, s1));
    ...
    default:
        my_log(LOG_INFO, 0, "ignoring unknown IGMP message type %x from %s to %s",
...
/**
**  Construct an IGMP message in the output packet buffer. The caller may have
**  already placed data in that buffer, of length 'datalen'.
*/
