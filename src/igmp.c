...
void acceptIgmp( int recvlen )
{
    ...
    if ( iphdrlen + ipdatalen != recvlen ) {
        my_log( LOG_WARNING, 0, "received packet from %s shorter (%u bytes) than hdr+data length (%u+%u)",
    ...
    my_log( LOG_NOTICE, 0, "RECV %s from %-15s to %s", igmp_packet_kind( igmp->igmp_type, igmp->igmp_code ),
            inetFmt( src, s1 ), inetFmt( dst, s2 ) );
    ...
    switch ( igmp->igmp_type ) {
        ...
        default:
            my_log( LOG_INFO, 0, "ignoring unknown IGMP message type %x from %s to %s", igmp->igmp_type,
                    inetFmt( src, s1 ), inetFmt( dst, s2 ) );
            return;
    }
}
...
void sendIgmp( uint32_t src, uint32_t dst, int type, int code, uint32_t group, int datalen )
{
    ...
    my_log( LOG_DEBUG, 0, "SENT %s from %-15s to %s", igmp_packet_kind( type, code ),
