...
**  callout.c
**
**  Implement callout queue and handle reoccuring tasks.
**
**----------------------------------------------------------------------------*/
...
/**
** Dumps the current queue state to the log.
*/
...
int timer_setTimer(int delay, timer_f action, void *data) {
    ...
    // if the queue is empty, insert node and return
    if (!queue) {
        queue = node;
    } else {
...
/**
** Shows ID and due time for queue entries.
** Used for debugging.
*/
static void debugQueue(void) {
    struct timeOutQueue *ptr;
    ...
    for (ptr = queue; ptr; ptr = ptr->next) {
        my_log(LOG_DEBUG, 0, "(Queue ID: %d, Time: %d) ", ptr->id, ptr->time);
    }
}
