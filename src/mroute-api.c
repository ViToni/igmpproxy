...
void delVIF( struct IfDesc *IfDp )
{
    ...
    if ( -1 == IfDp->vifindex ) {
        my_log( LOG_DEBUG, 0, "delVIF: No VIF to remove, Ix %d, %s (IP: %s)", IfDp->vifindex, IfDp->Name,
    ...
    */
    ...
    my_log( LOG_DEBUG, 0,
            "delVIF: Removing VIF (MRT_DEL_VIF), Ix %d Fl 0x%x IP 0x%08x %s, Threshold: %d, Ratelimit: %d",
...
void addVIF( struct IfDesc *IfDp )
{
    ...
    if ( NULL == VifDp ) {
        ...
        my_log( LOG_WARNING, 0, "addVIF: VIF #%d is already in use by IF %s (IP: %s)", VifDp - VifDescVc, IfDp->Name,
                inetFmt( IfDp->InAdr.s_addr, s1 ) );
    }
     ...
     */
    if ( NULL == VifDp ) {
        my_log( LOG_DEBUG, 0, "addVIF: No more VIFs! Could not add VIF, Ix %d, %s (IP: %s)", IfDp->vifindex, IfDp->Name,
                inetFmt( IfDp->InAdr.s_addr, s1 ) );
    ...
    for ( currSubnet = IfDp->allowednets; currSubnet; currSubnet = currSubnet->next ) {
        my_log( LOG_DEBUG, 0, "        Network for [%s] : %s", IfDp->Name,
    ...
    if ( setsockopt( MRouterFD, IPPROTO_IP, MRT_ADD_VIF, (char *) &VifCtl, sizeof( VifCtl ) ) ) {
        my_log( LOG_ERR, errno, "MRT_ADD_VIF: [%s]·:·%s", IfDp->Name, inetFmt( currSubnet->subnet_addr, s1 ) );
    }
...
int addMRoute( struct MRouteDesc *Dp )
{
...

    {
        ...
        my_log( LOG_DEBUG, 0, "Adding MFC (MRT_ADD_MFC): %s -> %s, InpVIf: %d", fmtInAdr( FmtBuO, CtlReq.mfcc_origin ),
                fmtInAdr( FmtBuM, CtlReq.mfcc_mcastgrp ), (int) CtlReq.mfcc_parent );
    }
...
int delMRoute( struct MRouteDesc *Dp )
{
...

    {
        ...
        my_log( LOG_DEBUG, 0, "Removing MFC (MRT_DEL_MFC): %s -> %s, InpVIf: %d",
                fmtInAdr( FmtBuO, CtlReq.mfcc_origin ), fmtInAdr( FmtBuM, CtlReq.mfcc_mcastgrp ),
