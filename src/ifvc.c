...
**  ifvc.c
**
**  Creates the inteface description verctor IfDescVc and rebuilds it if
...
void rebuildIfVc() {
    ...
    for (IfPt = IfVc; IfPt < IfEp; IfPt = IfNext) {
        ...
        // when IF becomes enabeld from downstream, call addVIF to enable its VIF
...
**  functions of the module will fail if they are called before the vector is build.
*/
void buildIfVc(void) {
    struct ifreq IfVc[sizeof(IfDescVc) / sizeof(IfDescVc[0])];
    struct ifreq *IfEp;
        ...
        for (IfPt = IfVc; IfPt < IfEp; IfPt = IfNext) {
            ...
            */
            ...
            my_log(LOG_DEBUG, 0, "buildIfVc: Interface %s Addr: %s, Flags: 0x%04x, Network: %s",
                    ...
                    IfDescEp->Flags, inetFmts(subnet, mask, s1));
...
/**
**  Returns a pointer to the IfDesc whose subnet matches the supplied IP address.
**  The IP must match a interfaces subnet, or any configured allowed subnet on a interface.
*/
