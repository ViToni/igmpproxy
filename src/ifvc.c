...
struct SubnetList *create_allowednet( struct ifaddrs *ifa )
{
    ...
    */
    if ( 0x10d1 == ifa->ifa_flags ) {
        my_log( LOG_TRACE, 0, "create_allowednet: Interface %s is PPP (Flags: 0x%04x)", ifa->ifa_name, ifa->ifa_flags );
    ...
    // Debug log the result...
    char FmtBu[32];
    my_log( LOG_TRACE, 0, "create_allowednet: Addr: %s, Network: %s", fmtInAdr( FmtBu, in_addr ),
...
**
*/
void buildIf( struct ifaddrs *ifa, struct IfDesc *Dp, int state )
{
    ...
    */
    ...
    // Debug log the result...
    my_log( LOG_DEBUG, 0, "buildIf: Interface %s Addr: %s, Flags: 0x%04x, Network: %s", Dp->Name,
            fmtInAdr( FmtBu, Dp->InAdr ), Dp->Flags,
     ...
     */
    {
        ...
        for ( ifa = ifap; ifa; ifa = ifa->ifa_next ) {
            ...
            if ( NULL == ifa->ifa_addr ) { // IF has no IP set, skipping
                ...
                my_log( LOG_DEBUG, 0, "rebuildIfVc: Interface is non-IP: %s, skipping", ifa->ifa_name );
            ...
            if ( ifa->ifa_addr->sa_family != AF_INET ) { // not an IP interface, skipping
                ...
                my_log( LOG_DEBUG, 0, "rebuildIfVc: Interface is non-IP: %s, sa_family: %s (%u), skipping",
                        ifa->ifa_name, sa_family_str, ifa->ifa_addr->sa_family );
...
void buildIfVc( void )
{
     ...
     */
    {
        ...
        for ( ifa = ifap; ifa; ifa = ifa->ifa_next ) {
            ...
            if ( NULL == ifa->ifa_addr ) { // IF has no IP set, skipping
                // Log the skipped interface...
                my_log( LOG_DEBUG, 0, "buildIfVc: Interface is non-IP: %s", ifa->ifa_name );
            ...
            if ( ifa->ifa_addr->sa_family != AF_INET ) { // not an IP interface, skipping
                ...
                // Log the skipped interface...
                my_log( LOG_DEBUG, 0, "buildIfVc: Interface is non-IP: %s, sa_family: %s (%u)", ifa->ifa_name,
                        sa_family_str, ifa->ifa_addr->sa_family );
