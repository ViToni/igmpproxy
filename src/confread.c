...
**  confread.c
**
**  Generic config file reader. Used to open a config file, and read the tokens
**  from it. The parser is really simple, and does no backlogging.
**  This means that no form of text escaping and qouting is currently supported.
**  '#' chars are read as start of comments, and the comment lasts until
**  a newline or EOF.
**---------------------------------------------------------------------------*/
...
int openConfigFile(char *filename) {
    ...
    // Reset buffer pointer and readsize
    bufPtr = 0;
    readSize = 0;
...
/**
**  Returns the next token from the config file.
**  The function returns NULL if there are no more tokens in the file.
*/
...
/**
**  Returns the currently active token, or NULL if no token is available.
*/
