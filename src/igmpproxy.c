...
static const char Usage[] = "Usage: igmpproxy [-h] [-d] [-v [-v]] <configfile>\n"
                            "\n"
...
int main( int ArgCn, char *ArgVc[] )
{
    ...
    do {
        ...
        if ( !Log2Stderr ) {
            ...
            if ( close( 0 ) < 0 || close( 1 ) < 0 || close( 2 ) < 0 || open( "/dev/null", 0 ) != 0 ||
                    dup2( 0, 1 ) < 0 || dup2( 0, 2 ) < 0 || setpgid( 0, 0 ) < 0 ) {
            ...
            if ( Dp->InAdr.s_addr && !( Dp->Flags & IFF_LOOPBACK ) ) {
                if ( Dp->state == IF_STATE_UPSTREAM ) {
                    if ( upsvifcount < MAX_UPS_VIFS - 1 ) {
                        ...
                        my_log( LOG_ERR, 0, "Cannot set VIF #%d as upstream as well. Max upstream Vif count is %d", Ix,
                                MAX_UPS_VIFS );
                    }
                }
